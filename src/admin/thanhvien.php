<?php 
     ...
     require( '../menu.php');
     // Tiện ích cấu trúc
	 $list = $DB->get_list("SELECT * FROM `users` ORDER BY money DESC ");    
?><!-- Nội dung -->
<div class="content">
	...
	<div class="card-body">
		<div class="row">
		  <h5 class="card-title fs-30 h4 title-pages">
			<img src="/assets/img/icon/6936667.png" width="45"> Quản lý thành viên
		  </h5> 
		</div>
		...
		<table class="table table-cen">
                        ...
                        foreach ($list as $users) { ?>
							...
							<tr>
								...
								<td>
								 <?php if($users['referral']==""){ echo "chưa có mã  giới thiệu"; }else{ echo $users['referral']; } ?>
							    </td>
								<td>
								 <?php if($users['phone']==""){ echo "chưa gắn số điện thoại"; }else{ echo $users['phone']; } ?>
							    </td>
								<td>
								 <?php if($users['email']==""){ echo "chưa gắn địa chỉ email"; }else{ echo $users['email']; } ?>
							    </td>
						    ...
						    <div class="card-box">
						     ...
						     <div class="form-group">
						       <label>email: </label>
						       <input type="text" name="email_<?=$users['id'];?>" value="<?=$users['email'];?>" class="form-control" placeholder="Nhập địa chỉ email" >
					         </div>

						     <div class="form-group">
						       <label>phone: </label>
						       <input type="text" name="phone_<?=$users['id'];?>" value="<?=$users['phone'];?>" class="form-control" placeholder="Nhập số điện thoại" >
					         </div>
							 ...
							 <div class="form-group">
						       <label>trạng thái: </label>
							    <div class="form-check form-switch">
                                  <input name="status_<?=$users['id'];?>"class="form-check-input check-input" type="checkbox" role="switch" id="flexSwitchCheckChecked" <?=checked($users['status']);?>>
                                </div>
					         </div>
						    </div>
						   </div>	
						  ...
						  $("#check_<?=$users['id'];?> div form").submit(function(e) {
                            e.preventDefault(); 
                               ...
                               curl(url,method,{
								   ...
								   'email':$('input[name="email_<?=$users['id'];?>"]').val(),
								   'phone':$('input[name="phone_<?=$users['id'];?>"]').val(),
